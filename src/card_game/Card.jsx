...
function Card({ Front, Back, cardDisplay }) {
  ...
  return (
    <>
      <div
        className={clsx(
          "absolute transition-all duration-150 select-none -m-1 border-4 rounded-2xl",
          hovering ? "border-blue-200" : "border-transparent"
        )}
        style={{
          top,
          left,
          zIndex: hovering ? 5000 : zIndex,
      ...
      >
        {visible && (
          <Face
            ...
            style={{ width, height, transform, transitionProperty: "width, height, transform" }}
          />
        )}
      </div>
      {/* Invisible hover target to improve behavior when hovering over occluded cards */}
      <div
        className={clsx("absolute transition-all duration-150 cursor-pointer")}
